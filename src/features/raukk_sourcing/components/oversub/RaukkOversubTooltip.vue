...
<template>
	<Teleport to="body">
		<div
			...
			ref="refTooltip"
			class="fixed z-50 pointer-events-none max-w-xs rounded border border-white/20 bg-black/90 px-3 py-2 text-xs text-white shadow-lg"
			:style="positionStyle">
			...
			<!-- teleported outside the section, so the status colors are
			 ...
			 Surface and border deliberately mirror `tooltipConfig` in
			 ui/styles.ts: a chart tooltip must not look like a second
			 kind of tooltip. -->
			<div
