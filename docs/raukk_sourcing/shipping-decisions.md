...
| 33 | rd 28 widened: assignment is an ACCOUNT GLOBAL input, not a display filter (USER, "unassigned plans shouldn't impact anything until reenabled") | `planHubSpokeRouting` read the raw snapshots while its mirror `subscription` read the scoped ones, so an unassigned base held an operated base's output on its exchange lane and no surface could show it — only visible through an LM sold ticker, where the `unitsPerDay − subscribedOf` term is gone and the `viaCxSold` remainder stands alone. Scoped now, plus the chain anchor lookup, the chain detail preview, and the depot / lease host / chain stop pickers. Pure label maps stay unscoped on purpose: an existing loop must still name a member it lost |
| 33 | changing the assignment stales the WHOLE store, and `deletePlanData` stales upstream as well as downstream | the dependency graph runs consumer → producer, so nothing ever staled the PRODUCER whose hub/spoke term a vanishing draw just changed — its frozen lanes kept flying cargo for a base that no longer existed. Same argument as `setShippingConfig`: an account global input moves every plan at once and no edge can carry it. The first non-empty assignment seen is the page load and stales nothing |
