# Shipping — decision residue (29 rounds)
...
| 29 | a draw whose source sits on the CONSUMER's own planet rides no pair | generalizes the self-sourcing exemption from "same plan" to "same planet", per origin — a mixed aggregate exempts only its local producer |
| 29 | a leased base ships nothing itself; its residual cargo freezes onto the snapshot and folds into the HOST's exchange pair | delegation moves the shipping, never re-resolves the sourcing — so staleness and lease-before-host ordering fall out of the existing graph edge |
| 29 | lease links never chain and never cross planets | one docking site is the whole premise; a chain would need a transshipment notion the model does not have |
...
- Mixed local+remote aggregates still book roughly full freight: the per-unit lane average spreads over all units (rd 29). Pure local draws are genuinely free.
- The lease's freight share raises the host's lane cost but lands in no plan's break-even price — site totals are right, per-unit attribution needs a second cross-plan read (rd 29).
